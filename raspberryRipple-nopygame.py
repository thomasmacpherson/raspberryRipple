...
def screenRefresh():
	...
	scoreText = font.render(score, 1, (200, 10, 10))
...
leds[current].turn_on()	#pfio.digital_write(current,1)			# turn colour on
		# turn off hit light
set_time = 900					# time allowed to hit each light (starts off large and reduced after each hit)
...
while True:
	...
	if full_input != previous_pressed:		# check this is a new button press
		...
		print ("Missed one!")
		score -= 1
		if misses >= 5:					# too many misses = Game Over!
			break
